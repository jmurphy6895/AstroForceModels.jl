    ...
    for backend in _BACKENDS
        ...
        @testset "$testname" begin
            f_fd, df_fd = value_and_jacobian(
                (x) -> acceleration(x, p, t, sun_third_body), AutoFiniteDiff(), state
            )

            f_ad, df_ad = value_and_jacobian(
                (x) -> Array(acceleration(x, p, t, sun_third_body)), backend[2], state
            )
    ...
    for backend in _BACKENDS
        ...
        @testset "$testname" begin
            f_fd, df_fd = value_and_derivative(
                (x) -> acceleration(state, p, x, sun_third_body), AutoFiniteDiff(), t
            )

            f_ad, df_ad = value_and_derivative(
                (x) -> Array(acceleration(state, p, x, sun_third_body)), backend[2], t
            )
