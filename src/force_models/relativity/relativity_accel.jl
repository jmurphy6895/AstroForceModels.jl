...
function acceleration(
    ...
    return relativity_accel(u, relativity_model.central_body.μ, relativity_model.sun_body.μ, sun_pos, sun_vel, p.c, J)
...
function relativity_accel(
    ...
    schwartzchild = μ_body/((c^2.0)*(r_norm^3.0)) * (((2.0*(β + γ))*(μ_body/r_norm) - γ*dot(ṙ, ṙ))*r + 2.0*(1.0 + γ)*dot(r, ṙ)*ṙ)

    lense_thirring = (1.0 + γ)*(μ_body/((c^2.0)*(r_norm^3.0)))*((3.0/r_norm)*cross(r, ṙ)*dot(r, J) + cross(ṙ, J))

    de_sitter = (1.0 + 2.0*γ)*cross(cross(v_sun, (-μ_Sun/((c^2.0)*(r_norm^3.0)))*r_sun), ṙ)
